  ...
  module Checkout
    class Walkthrough
      def self.run(client)
        ...
        # Any stock locations that exist, must be marked to not propagate

        response = client.post('/api/products?product[name]=Headphones&product[price]=100&product[shipping_category_id]=1')
        if response.status == 201
          client.succeeded "Created new product."
          product = JSON.parse(response.body)
          product_id = product['id']
          variant_id = product['master']['id']
        else
          client.failed 'Failed to create a new product.'
        end

        response = client.put("/api/products/#{product_id}/variants/#{variant_id}",
        {
          variant: {
        ...
        if response.status == 200
          client.succeeded "Product updated to not track inventory"
        else
          client.failed 'Failed to not track inventory.'
        end
